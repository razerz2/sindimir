...
@section('title', 'Categorias de curso')

@section('subtitle')
    Gestao do catalogo de categorias.
@endsection

@section('content')
    <div class="page-actions">
        <div></div>
        <a class="btn btn-primary" href="{{ route('admin.catalogo.categorias.create') }}">Nova categoria</a>
    </div>
            ...
            <tbody>
                @forelse ($categorias as $categoria)
                    <tr>
                        ...
                        <td>
                            <span class="badge {{ $categoria->ativo ? '' : 'warning' }}">
                                {{ $categoria->ativo ? 'Ativo' : 'Inativo' }}
                            </span>
                        </td>
                        <td>
                            <div class="table-actions">
                                <a class="btn btn-ghost" href="{{ route('admin.catalogo.categorias.show', $categoria) }}">
                                ...
                                <a class="btn btn-ghost" href="{{ route('admin.catalogo.categorias.edit', $categoria) }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="1.6">
                                ...
                                <form action="{{ route('admin.catalogo.categorias.toggle', $categoria) }}" method="POST" style="display:inline">
                                    @csrf
                                    <button class="btn btn-ghost" type="submit">
                                        ...
                                        <span>{{ $categoria->ativo ? 'Inativar' : 'Ativar' }}</span>
                                    </button>
                                </form>
                                <form action="{{ route('admin.catalogo.categorias.destroy', $categoria) }}" method="POST" style="display:inline">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-danger" type="submit" @if ($categoria->cursos_count > 0) disabled @endif>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none"
                                            stroke="currentColor" stroke-width="1.6">
