...
class SiteSectionUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $tipo = $this->input('tipo', $section?->tipo);
        $isHomeSlot = $currentSlug && in_array($currentSlug, SiteSection::HOME_SLOTS, true);
        $expectedType = $isHomeSlot ? (SiteSection::HOME_TYPES[$currentSlug] ?? $section?->tipo) : null;
        ...
        return [
            'slug' => [
                ...
                'string',
                'max:255',
                Rule::unique('site_sections', 'slug')->ignore($sectionId),
                Rule::when($isHomeSlot, Rule::in([$currentSlug])),
            ],
